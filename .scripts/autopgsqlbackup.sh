...
DBNAMES=`psql postgres -t -c "SELECT datname FROM pg_database WHERE datname NOT IN ('template0','template1');"`
# DBNAMES="all"

# Backup directory location e.g /backups
BACKUPDIR="/Users/joao/Cloud/GDrive/Backup/Archives/PGSQL"
...
MAILADDR="joao.ponceleao@gmail.com"
...
DBEXCLUDE="template1 template0 data"
...
# Choose Compression type. (gzip or bzip2 or xz or tar (for directories))
COMP="bzip2"

# Choose dump type. (file or dir)
DTYPE="file"
...
if [ "$DTYPE" = "dir" ]; then
    DARGS="--format=d --username=$USERNAME $HOST $OPT $1 -f $2"
else
    DARGS="--username=$USERNAME $HOST $OPT $1 > $2"
fi
...
if [ "$COMP" = "gzip" ]; then
	...
	echo Compression information for "$1"
	lbzip2 $1
	SUFFIX=".bz2"
elif ["$COMP" = "xz" ]; then
    pixz "$1"
	...
	echo Compression information for "$1"
    tar cjf --use-compress-program lbzip2 "$1.tar.bz2" "$1"
	SUFFIX=".bz2"
else
...
echo ======================================================================
	...
	if [ $DOM = "01" ]; then
        noti -t "PG Backup" -m "Starting monthly backup of $DB."
		for MDB in $MDBNAMES
		do
        ...
        noti -t "PG Backup" -m "Monthly backup of $DB finished."
	fi
	...
	if [ $DNOW = $DOWEEKLY ]; then
        noti -t "PG Backup" -m "Starting weekly backup of $DB."
		...
		echo ----------------------------------------------------------------------
        noti -t "PG Backup" -m "Weekly backup of $DB finished."

	# Daily Backup
	else
        noti -t "PG Backup" -m "Starting daily backup of $DB."
		...
		echo ----------------------------------------------------------------------
        noti -t "PG Backup" -m "Daily backup of $DB finished."
	fi
	done
